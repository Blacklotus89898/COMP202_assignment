...
def special_orders(): 
    ...
    The cost is $2592.95
     >>> special_orders()
    Would you like the cake or the poutine? poutine
    Enter diameter: 3
    Enter height: 10
    Do you want guacamole? n
    The cost is $212.06
     >>> special_orders()
    Would you like the cake or the poutine? cake
    Enter diameter: 9
    ...
    if input("Would you like the cake or the poutine? ") == "cake":
        cake_cost = get_pizza_cake_cost(float(input("Enter diameter: ")), float(input("Enter height: ")))
        a = input("Do you want "+ SPECIAL_INGREDIENT +"? ")
        if a == "y" or a == "yes":
            price = cake_cost+SPECIAL_INGREDIENT_COST
        else:
            price = cake_cost
    else:
        poutine_cost = get_pizza_poutine_cost(float(input("Enter diameter: ")), float(input("Enter height: ")))
        a = input("Do you want "+SPECIAL_INGREDIENT +"? ")
        if a == "y" or a == "yes":
            price = poutine_cost+SPECIAL_INGREDIENT_COST
        else:
            price = poutine_cost
...
def quantity_mode():
    ...
    >>> quantity_mode()
    Enter diameter 1: 24
    Enter diameter 2: 12.3
    You should buy 4 pizzas.
    """
    x=float(input("Enter diameter 1: "))
    y=float(input("Enter diameter 2: "))
...
def formula_mode():
    ...
    The missing value is 3.87
     >>> formula_mode()
    Enter large diameter: 6
    Enter small diameter: 3
    Enter large price: -1
    Enter small price: 8
...
def run_pizza_calculator():
    ...
    If the input is not matched to a function, the function prints Invalid mode.
